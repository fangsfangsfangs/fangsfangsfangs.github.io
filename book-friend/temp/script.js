...
async function renderQuickListCard() {
  try {
    ...
    const listItems = data
      .map(
        (book) =>
          `<div class="quicklist-item"><strong>${(book.title || "Untitled").trim()}</strong><br><em>${(book.author || "Unknown").trim()}</em></div>`
      )
      .join("");
    ...
    openContentOverlay(
      `<p style="color:red;">Failed to load quick list.</p><button class="close-btn" data-close>×</button>`
    );
  }
}
