...
async function fetchEnrichedBookData(title, author) {
  console.log(`Enriching data for: ${title}`);
  try {
    // Fetch ISBN and Subject tags in parallel to save time
    const [isbn, tags] = await Promise.all([fetchIsbn(title, author), fetchFilteredSubjects(title, author)]);

    // Now that we have the best ISBN, fetch the cover URL
    const coverUrl = await getCoverUrl({ title, author, isbn });
    ...
    console.error("Failed to enrich book data:", error);
    // Return a default object so the app doesn't crash
    return { isbn: "", coverUrl: "", tags: [] };
  }
}
